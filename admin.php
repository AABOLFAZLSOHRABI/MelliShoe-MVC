                    ...
                    <div class="p-8 md:p-10">
                        ...
                        if (isset($_POST['submit'])) {
                            if (isset($_POST['email']) && !empty($_POST['email']) && isset($_POST['password']) && !empty($_POST['password'])) {
                                $query = $db->prepare('SELECT id, password_hash FROM admins WHERE email = ? ');
                                $query->execute([$_POST['email']]);
                                $data = $query->fetch();
                                if ($data && password_verify($_POST['password'], $data['password_hash'])) {
                                    $_SESSION['admin_panel'] = $data['id'];
                                    header('Location: dashboard.php');
                                    exit();
                                } else {
                                    echo "<p class='text-red-500'>ایمیل یا رمز عبور اشتباه است.</p>";
                                }
                            } else {
                                // Show error message
                                echo "<p class='text-red-500'>لطفاً تمام فیلدها را پر کنید.</p>";
                            }
                        } else {
                        ...
                        ?>
                        <!-- Login Form -->
                        <form class="flex flex-col gap-6" action="admin.php" method="post">
                            <!-- Email input field -->
                            ...
                            <!-- Additional options: remember me + forgot password (design hints from reference) -->
                            <div class="flex items-center justify-between gap-4 flex-wrap">
                                <label
                                    class="flex cursor-pointer items-center gap-2 text-[14px] leading-5 tracking-[-0.48px] text-[#242424] transition-all hover:opacity-80">
                                    <input type="checkbox"
                                        class="h-4 w-4 rounded border-[#a5a5a5] text-[#00286d] focus:ring-1 focus:ring-[#00286d]/30" />
                                    <span>مرا به خاطر بسپار</span>
                                </label>
                                <a href="#"
                                    class="text-[14px] leading-5 tracking-[-0.48px] text-[#00286d] transition-opacity duration-150 hover:opacity-70 font-medium">رمز
